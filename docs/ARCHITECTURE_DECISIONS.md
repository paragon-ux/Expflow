...
| AD-021 | Durable Gate C records are validated against the applicable schema constraints before immutable writes.                                                         | Authority, semantic, workflow, projection, regeneration, equivalence, and reuse records are long-lived state; invalid records must not be persisted. | Gate C records        |
| AD-022 | Current authority sources are derived from accepted registration decisions, source supersession, and effective intervals.                                       | A stale, expired, or superseded source must not remain current solely because it once had an acceptance decision.                                    | Authority model       |
