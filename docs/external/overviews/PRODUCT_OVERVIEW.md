...
Expflow `v1.0.1` proves the material core, but it does not yet provide a complete ordinary decision surface. The current four-command CLI can initialize a project, record complete material-tree revisions, report drift, and restore recorded node or tree state as a new forward revision. Independent `v1.0.1` testing also confirms several usability and safety limits that Build Week Phase 1 must correct:
