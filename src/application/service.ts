...
function genToken(): string { return `efp_${randomBytes(16).toString('base64url').slice(0, 24)}`; }
...
export class ApplicationService {
  ...
  private root() { return this.projectRoot; }
  ...
  async init(actor: Actor): Promise<ApplicationResult<{ projectId: string }>> {
    ...
    } catch (e: unknown) { const m = e instanceof Error ? e.message : 'Init failed'; return blocked('init', actor, 'OPERATION_FAILED', m); }
  }

  async inspect(actor: Actor): Promise<ApplicationResult<unknown>> {
    try { const s = await this.rt.status({ root: this.root() }); return ok('inspect', actor, s); }
    catch (e: unknown) { const m = e instanceof Error ? e.message : 'Inspect failed'; return blocked('inspect', actor, 'OPERATION_FAILED', m); }
  }
  ...
  async status(actor: Actor, opts?: CommandOptions): Promise<ApplicationResult<unknown>> {
    try {
      const s = await this.rt.status({ root: this.root(), history: opts?.dryRun ? true : undefined });
      return ok('status', actor, s);
    } catch (e: unknown) { const m = e instanceof Error ? e.message : 'Status failed'; return blocked('status', actor, 'OPERATION_FAILED', m); }
  }
  ...
  async applySync(actor: Actor, plan: Plan<unknown>): Promise<ApplicationResult<{ receiptId: string }>> {
    try {
      const r = await this.rt.sync({ root: this.root(), expectedHead: plan.expectedHead ?? undefined });
      return ok('applySync', actor, { receiptId: r.operation_id }, r.operation_id);
    } catch (e: unknown) { const m = e instanceof Error ? e.message : 'Sync failed'; return blocked('applySync', actor, 'OPERATION_FAILED', m); }
  }

  async sync(actor: Actor, opts?: CommandOptions): Promise<ApplicationResult<{ receiptId: string }>> {
    try {
      const r = await this.rt.sync({ root: this.root(), expectedHead: opts?.expectedHead ?? undefined, dryRun: opts?.dryRun });
      return ok('sync', actor, { receiptId: r.operation_id }, r.operation_id);
    } catch (e: unknown) { const m = e instanceof Error ? e.message : 'Sync failed'; return blocked('sync', actor, 'OPERATION_FAILED', m); }
  }
  ...
  async planRestore(actor: Actor, reference: string): Promise<ApplicationResult<Plan<unknown>>> {
    try {
      ...
      return ok('planRestore', actor, { token: genToken(), expectedHead: '', operation: 'restore', plan: p, createdAt: ts() });
    } catch (e: unknown) { const m = e instanceof Error ? e.message : 'Restore plan failed'; return blocked('planRestore', actor, 'OPERATION_FAILED', m); }
  }

  async applyRestore(actor: Actor, reference: string, _plan?: Plan<unknown>): Promise<ApplicationResult<{ receiptId: string }>> {
    try {
    ...
    } catch (e: unknown) { const m = e instanceof Error ? e.message : 'Restore failed'; return blocked('applyRestore', actor, 'OPERATION_FAILED', m); }
  }

  async restore(actor: Actor, reference: string, opts?: CommandOptions): Promise<ApplicationResult<{ receiptId: string }>> {
    try {
