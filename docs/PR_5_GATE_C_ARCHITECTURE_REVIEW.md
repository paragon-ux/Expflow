...
**Reviewed head:** `7df328d7883428942620301778a1e359346533b1`
**Resolution implementation commit:** `206c2ab0ea5af68ccc734319562920d02a6e4f5f`
...
The original review blocked Gate C because the implementation overclaimed AD-021 and AD-022, projection-head semantics were underspecified, and mutable evidence docs were stale. The implementation commit `206c2ab0ea5af68ccc734319562920d02a6e4f5f` fixes F1-F6 and adds regression coverage for each behavior. PR #5 remains stacked on PR #4, so hosted-check visibility depends on the PR base configuration.
...
| F2  | fixed        | `src/core/record-validation.ts`, `src/semantics/runtime.ts`, `tests/unit/gate-c-runtime.test.ts`                                                                                                      | Nested attribution and semantic claim objects reject extra keys and missing required claim values before persistence.                       |
...
AD-021 is upheld after F1, F2, and F3. The runtime now distinguishes required and optional digest checks, rejects nested semantic shape drift, and blocks accepted workflow completion without a decision reference.
...
Merge-ready locally after final validation. PR #5 is still stacked on PR #4, so the remaining remote caveat is hosted-check/base behavior rather than an unresolved Gate C implementation blocker.
...
| ------------------------------------------------------------------------------------ | --------: | -------------- |
| `npm test -- tests/unit/authority-runtime.test.ts tests/unit/gate-c-runtime.test.ts` |         0 | PASS, 14 tests |
